...
public class WinResultAdapter extends RecyclerView.Adapter<WinResultAdapter.WinResultHolder> {
    ...
    private List<AwardPersonInfo> list;

    public WinResultAdapter(Context context, List<AwardPersonInfo> list) {
    ...
    public WinResultHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.layout_win_result_list, parent, false);
    ...
    public void onBindViewHolder(WinResultHolder holder, int position) {
        ...
        AwardPersonInfo personInfo = list.get(position);
        ...
        holder.ivWinResultHead.setImageURI(personInfo.pictureUrl);
        holder.tvWinResultPlayerName.setText(personInfo.userName);
        holder.tvWinResultPrizeInfo.setText(personInfo.prizeName);
    }

    public void refreshAdapterData(List<AwardPersonInfo> listInfo) {
        list.clear();
        list.addAll(listInfo);
        notifyDataSetChanged();
    }

    public void loadMoreAdapterData(List<AwardPersonInfo> infoList) {
