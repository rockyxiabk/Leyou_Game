...
public class FriendDetailActivity extends BaseActivity implements IFriendDetail {
    ...
    private void setFriendUI() {
        ivContactHeader.setImageURI(friend.getHeadImgUrl());
        tvName.setText(!TextUtils.isEmpty(friend.getComment()) ? friend.getComment() : friend.getNickname());
        tvNickName.setText("昵称：" + friend.getNickname());
        tvFriendPhoneNumber.setText(friend.getPhone());
        tvFriendAddress.setText(friend.getRegion());
        ...
        switch (friend.getStatus()) {
            ...
            case Friend.FRIEND://我的好友
                ...
                break;
            case Friend.ADDING_FRIEND_REFUSED://是否通过验证
                btnSendMessage.setVisibility(View.GONE);
                btnFriendStatus.setVisibility(View.VISIBLE);
                btnFriendStatus.setText("已拒绝");
                btnFriendDelete.setVisibility(View.GONE);
                break;
            case Friend.ADDING_FRIEND_PASS_VERIFY://是否通过验证
                btnSendMessage.setVisibility(View.GONE);
                btnFriendStatus.setVisibility(View.VISIBLE);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.re_set_remark:
                if (null != friend && !TextUtils.isEmpty(friend.getComment())) {
                    EditTextDialog editDialog = new EditTextDialog(this, "修改备注", TextUtils.isEmpty(friend.getComment()) ? "" : friend.getComment(), getResources().getString(R.string.confirm));
                    editDialog.setOnConfirmListener(new EditTextDialog.OnConfirmListener() {
                        @Override
                        public void onFinished(String name) {
                            friend.setComment(name);
                            setFriendUI();
                    ...
                    EditTextDialog editDialog = new EditTextDialog(this, "设置备注", "", getResources().getString(R.string.confirm));
                    editDialog.setOnConfirmListener(new EditTextDialog.OnConfirmListener() {
                        @Override
                        public void onFinished(String name) {
                            friend.setComment(name);
            ...
            case R.id.btn_friend_status:
                switch (friend.getStatus()) {
                    ...
                    case Friend.FRIEND:
                        break;
                    case Friend.ADDING_FRIEND_PASS_VERIFY:
                        presenter.agreeAddFriend(friend, 1);
                        break;
                    case Friend.ADDING_FRIEND_REFUSED:
                        presenter.agreeAddFriend(friend, 0);
                        break;
            ...
            case R.id.btn_friend_delete:
                String string = TextUtils.isEmpty(friend.getName()) ? friend.getNickname() : friend.getName();
                final CustomDialog customDialog = new CustomDialog(this, getResources().getString(R.string.friend_delete),
                        "确定将联系人 \"" + string + "\" 删除，同时删除与该联系人的所有聊天记录", getResources().getString(R.string.cancel), getResources().getString(R.string.delete));
    ...
    public void setFriendInfo(Friend friend) {
        if (null != friend) {
            if (friend.getStatus() == 3) {
                this.friend = DBUtil.getInstance(this).queryFriendByUserId(friend.getUserId());
            } else {
                PhoneContact phoneContact = DBUtil.getInstance(this).queryPhoneContactByUserId(friend.getUserId());
                this.friend.setName(phoneContact.getName());
                this.friend.setPhone(phoneContact.getPhone());
                this.friend.setRegion(friend.region);
                this.friend.setStatus(phoneContact.getStatus());
                this.friend.setNickname(phoneContact.getNickname());
                this.friend.setHeadImgUrl(phoneContact.getHeadImgUrl());
                this.friend.setPhoneNameLetter(phoneContact.getPhoneNameLetter());
            }
            setFriendUI();
            RongUserInfoManager.getInstance().setUserInfo(new UserInfo(friend.getUserId(), friend.getNickname(), Uri.parse(friend.getHeadImgUrl())));
        }
    }
