...
public class PhoneContactsActivity extends BaseActivity implements IPhoneContacts, CustomSlideBar.OnTouchingLetterChangedListener, CustomItemClickListener {
    ...
    Button btnSetting;
    private List<PhoneContact> friends = new ArrayList<>();
    ...
    private void initData() {
        ...
        if (AndPermission.hasPermission(this, Manifest.permission.READ_CONTACTS)) {
            ...
            int days = 0;
            if (currentTime > 0) {
                days = DataUtil.distanceDay(currentTime, System.currentTimeMillis());
            } else {
                days = 1;
            }
            if (days >= 1) {//超过一天 没有更新
                ...
                friends = DBUtil.getInstance(this).queryPhoneContactList();
                if (null != friends && friends.size() > 0) {
                    changeLoadingDes("读取联系人数据...");
                    presenter.getMyBookPhoneList();
                } else {
                    showListOrErrorView(false);
                }
                dismissedLoading();
    ...
    @Override
    public void showPhoneContactsList(List<PhoneContact> friends) {
        showListOrErrorView(true);
        dismissedLoading();
        Collections.sort(friends, new PhoneContactComparator());
        this.friends = friends;
        adapter.updateAdapter(this.friends);
    }

    @Override
    public void showNetContactSList(final List<PhoneContact> friend) {
        showListOrErrorView(true);
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                List<PhoneContact> friends = DBUtil.getInstance(PhoneContactsActivity.this).queryPhoneContactList();
                Collections.sort(friends, new PhoneContactComparator());
                PhoneContactsActivity.this.friends.clear();
                PhoneContactsActivity.this.friends.addAll(friends);
                adapter.updateAdapter(PhoneContactsActivity.this.friends);
                dismissedLoading();
    ...
    public void onItemClick(View view, int position) {
        PhoneContact friend = friends.get(position);
