...
public class FriendRankActivity extends BaseFragmentActivity implements IFriendRankActivity, TabLayout.OnTabSelectedListener {
    ...
    @OnClick({R.id.iv_win_result_back, R.id.ivbtn_friend_rank_explain})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.iv_win_result_back:
                finishCurrentActivity();
                break;
            case R.id.ivbtn_friend_rank_explain:
    ...
    private void setFragment() {
        for (int i = 0; i < list.size(); i++) {
            ...
            fragmentList.add(FriendRankFragment.newInstance(gamePrizeResult.name, gamePrizeResult.uniqueMark));
