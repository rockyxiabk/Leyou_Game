...
public class FriendDetailActivityPresenter {
    ...
    public void setRemarkName(final String userId, final String remarkName) {
        HttpUtil.subscribe(HttpUtil.createApi(FriendApi.class, Constants.URL).editFriendComment(userId, remarkName), new Observer<Result<Friend>>() {
            @Override
            public void onCompleted() {
            ...
            @Override
            public void onNext(Result<Friend> result) {
                if (result.result == 1) {
                    Friend friend = DBUtil.getInstance(context).queryFriendByUserId(userId);
                    friend.setComment(remarkName);
    ...
    private void getUserPlayedGame(String userId) {
        HttpUtil.subscribe(HttpUtil.createApi(GameApi.class, Constants.URL).getMyFriendsPlayedGames(userId), new Observer<ResultArray<GameBean>>() {
            @Override
            public void onCompleted() {
