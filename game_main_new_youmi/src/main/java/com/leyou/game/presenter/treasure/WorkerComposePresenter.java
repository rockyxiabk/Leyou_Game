...
public class WorkerComposePresenter {
    ...
    private WorkerBean worker1;
    private WorkerBean worker2;
    private Observer<Result<WorkerExtBean>> observer = new Observer<Result<WorkerExtBean>>() {
        @Override
    ...
    public void composeWorker(final WorkerBean worker1, WorkerBean worker2) {
        ...
        subscribe = HttpUtil.subscribe(HttpUtil.createApi(TreasureApi.class, Constants.TREASURE_URL)
                .composeWorker(worker1.id, worker2.id), observer);
    }
