...
public class WorkerUpgradeActivity extends BaseActivity implements IWorkerUpgradeActivity, CustomItemClickListener {
    ...
    @BindView(R.id.iv_upgrade_explain)
    ImageButton tvUpgradeExplain;
    ...
    @BindView(R.id.tv_one_key_update)
    TextView tvOnwKeyUpdate;
    ...
    public void initWeight(Bundle savedInstanceState) {
        ...
        tvOnwKeyUpdate.getPaint().setFlags(Paint.UNDERLINE_TEXT_FLAG | Paint.ANTI_ALIAS_FLAG);//添加文字下划线
    ...
    @OnClick({R.id.iv_upgrade_back, R.id.iv_upgrade_explain, R.id.btn_upgrade_worker_confirm, R.id.tv_one_key_update})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.iv_upgrade_back:
                finishCurrentActivity();
                break;
            case R.id.iv_upgrade_explain:
            ...
            case R.id.btn_upgrade_worker_confirm:
                ...
                switch (upgradeWorkerBean.typeId) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        des = "蓝色矿工每次升级消耗" + upgradeWorkerBean.levelUpConsume + "颗钻石（武力值随机增加5~10），确定要升级当前选择的矿工吗？";
                        break;
                    case 4:
                ...
                break;
            case R.id.tv_one_key_update:
                if (upgradeWorkerBean.attribute >= upgradeWorkerBean.maxAttribute) {
                    showMessageToast("当前矿工已经满级");
                } else {
                    String oneKeyDes = "一键升级，消耗更少的钻石，一键获取最大的武力值～";
                    switch (upgradeWorkerBean.typeId) {
                        case 0:
                            oneKeyDes = "本次升级需要消耗" + ((int) ((Math.ceil((upgradeWorkerBean.maxAttribute - upgradeWorkerBean.attribute) / 2.0)) * upgradeWorkerBean.levelUpConsume)) + "钻石，确定一键升至满级？";
                            break;
                        case 1:
                            oneKeyDes = "本次升级需要消耗" + ((int) ((Math.ceil((upgradeWorkerBean.maxAttribute - upgradeWorkerBean.attribute) / 3.5)) * upgradeWorkerBean.levelUpConsume)) + "钻石，确定一键升至满级？";
                            break;
                        case 2:
                            oneKeyDes = "本次升级需要消耗" + ((int) ((Math.ceil((upgradeWorkerBean.maxAttribute - upgradeWorkerBean.attribute) / 5.5)) * upgradeWorkerBean.levelUpConsume)) + "钻石，确定一键升至满级？";
                            break;
                        case 3:
                            oneKeyDes = "本次升级需要消耗" + ((int) ((Math.ceil((upgradeWorkerBean.maxAttribute - upgradeWorkerBean.attribute) / 7.5)) * upgradeWorkerBean.levelUpConsume)) + "钻石，确定一键升至满级？";
                            break;
                        case 4:
                            oneKeyDes = "本次升级需要消耗" + ((int) ((Math.ceil((upgradeWorkerBean.maxAttribute - upgradeWorkerBean.attribute) / 15.0)) * upgradeWorkerBean.levelUpConsume)) + "钻石，确定一键升至满级？";
                            break;
                    }
                    final CustomDialog customDialog = new CustomDialog(this, "一键升级", oneKeyDes, getString(R.string.cancel), "一键升级");
                    customDialog.setCustomListener(new CustomDialog.CustomListener() {
                        ...
                        public void confirm() {
                            customDialog.dismiss();
                            presenter.oneKeyUpgradeWorker(upgradeWorkerBean);
                        }
                    });
                    customDialog.show();
                }
