...
public class ConversationActivity extends FragmentActivity implements IConversationList, RongIM.UserInfoProvider, RongIM.GroupInfoProvider {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Constants.setCurrentConversationId(targetId);
    ...
    private void initTitle() {
        if (conversation.contains(Conversation.ConversationType.PRIVATE.getName().toLowerCase())) {
            Constants.setConversationType(Conversation.ConversationType.PRIVATE);
            ...
            if (null != friend) {
                title = !TextUtils.isEmpty(friend.getComment()) ? friend.getComment() : friend.getNickname();
            } else {
            ...
            Constants.setConversationType(Conversation.ConversationType.CHATROOM);
            ivConversationDetail.setImageResource(R.mipmap.icon_friend_crowd);
            conversationType = 4;
    ...
    public void setFriendInfo(Friend friend) {
        ...
        this.friend = friend;
        tvConversationTitle.setText(!TextUtils.isEmpty(friend.getComment()) ? friend.getComment() : friend.getNickname());
    ...
    public void setCrowdInfo(boolean flag, Crowd crowd) {
        if (flag) {
            ...
            this.crowd = crowd;
            if (!TextUtils.isEmpty(crowd.getName())) {
                if (crowd.getName().length() > 10) {
                    tvConversationTitle.setText("群聊(" + crowd.getMemberNum() + ")");
                } else {
                    tvConversationTitle.setText(crowd.getName());
                }
            } else {
                tvConversationTitle.setText("群聊");
            }
            if (crowd.getIsShielding() == 1) {
