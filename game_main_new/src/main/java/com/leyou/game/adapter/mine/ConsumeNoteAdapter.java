...
public class ConsumeNoteAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private final List<DiamondBean> list;
    private int currentNumber = 1;
    private boolean isLoadAllData = false;

    public ConsumeNoteAdapter(Context context, List<DiamondBean> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ConsumeHolder) {
            ...
            DiamondBean consumeBean = list.get(position);
            Glide.with(context).load(consumeBean.imgUrl).into(((ConsumeHolder) holder).ivConsumeIcon);
            consumeHolder.tvconsumeListTitle.setText(consumeBean.title);
            consumeHolder.tvConsumeTime.setText(DataUtil.getConvertResult(consumeBean.createTime, DataUtil.Y_M_D));
            consumeHolder.tvDiamondNumber.setText("" + consumeBean.num + "钻");
            if (!TextUtils.isEmpty(consumeBean.money)) {
                consumeHolder.tvMoney.setVisibility(View.VISIBLE);
                consumeHolder.tvMoney.setText(consumeBean.money);
            } else {
    ...
    public void addAdapter(List<DiamondBean> currentList) {
        list.clear();
        list.addAll(currentList);
        notifyDataSetChanged();
    }

    public void loadMoreData(List<DiamondBean> currentList) {
        list.addAll(currentList);
        notifyDataSetChanged();
    }
